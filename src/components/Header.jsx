        ...
        <nav class="flex items-center justify-between flex-wrap bg-cyan-400 p-6 mb-12">
          ...
          <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
            <div class="text-3xl lg:flex-grow">
              <div class={`block mt-4 lg:inline-block lg:mt-0 text-indigo-100 hover:text-white mr-4 ${isActive('/') ? 'active' : ''}`}>
                <Link to="./">About</Link>
              </div>
              <div class={`block mt-4 lg:inline-block lg:mt-0 text-indigo-100 hover:text-white mr-4 ${isActive('/Projects') ? 'active' : ''}`}>
                <Link to="./Projects">Projects</Link>
              </div>
              <div class={`block mt-4 lg:inline-block lg:mt-0 text-indigo-100 hover:text-white ${isActive('/Contact') ? 'active' : ''}`}>
               <Link to="./Contact">Contact</Link>
              </div>
            </div>
            <div>
              <div href="#" class={`inline-block text-sm px-4 py-2 leading-none border rounded text-white 
                border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0 ${isActive('/Resume') ? 'active' : ''}`}>
